...
readonly DEFAULT_REPORT_DIR="./mvnimble-results"      # Default directory for reports
...
# Function to add a print_env_summary function that can be called from other scripts
function print_env_summary() {
  echo "MVNimble Environment Information:"
  echo " - Version: ${MVNIMBLE_VERSION}"
  echo " - Build Date: ${MVNIMBLE_BUILD_DATE}"
  echo " - Operating System: $(uname -s) $(uname -r)"
  
  if command -v java &>/dev/null; then
    echo " - Java Version: $(java -version 2>&1 | grep version | awk '{print $3}' | tr -d '"')"
  else
    echo " - Java Version: Not found"
  fi
  
  if command -v mvn &>/dev/null; then
    echo " - Maven Version: $(mvn --version 2>/dev/null | head -n 1 | awk '{print $3}')"
  else
    echo " - Maven Version: Not found"
  fi
}