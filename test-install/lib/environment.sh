...
# Check if common.sh has been loaded, if not load it
...
function detect_environment() {
  ...
  # Set environment variables for reference by other scripts
  export MVNIMBLE_ENV_TYPE="$env_type"
  export MVNIMBLE_CPU_COUNT="$cpu_count"
  export MVNIMBLE_MEM_MB="$total_memory"
  export MVNIMBLE_DISK_MB="$disk_space"
...
function generate_environment_report() {
  ...
  # Generate the report
  {
    ...
    echo "MVNimble Library Directory: $SCRIPT_DIR"
    echo ""
  } > "$env_report"
